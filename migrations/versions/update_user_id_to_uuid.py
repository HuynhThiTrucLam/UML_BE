...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE personal_infor_documents DROP CONSTRAINT personal_infor_documents_user_id_fkey")
    
    # Step 2: Add temporary UUID columns to all tables that reference users.id
    op.execute("ALTER TABLE students ADD COLUMN temp_user_id uuid DEFAULT gen_random_uuid()")
    op.execute("ALTER TABLE staffs ADD COLUMN temp_user_id uuid DEFAULT gen_random_uuid()")
    op.execute("ALTER TABLE complaints ADD COLUMN temp_user_id uuid DEFAULT gen_random_uuid()")
    op.execute("ALTER TABLE personal_infor_documents ADD COLUMN temp_user_id uuid DEFAULT gen_random_uuid()")
    ...
    op.execute("ALTER TABLE personal_infor_documents RENAME COLUMN temp_user_id TO user_id")
    ...
    op.execute("ALTER TABLE staffs ADD CONSTRAINT staffs_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id)")
    op.execute("ALTER TABLE complaints ADD CONSTRAINT complaints_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id)")
    op.execute("ALTER TABLE personal_infor_documents ADD CONSTRAINT personal_infor_documents_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id)")
