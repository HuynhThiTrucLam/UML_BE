...
def upgrade() -> None:
    ...
    if 'current_students' not in columns:
        op.add_column('courses', sa.Column('current_students', sa.Integer(), nullable=False,
                                         server_default='0'))
        
    if 'price' not in columns:
        op.add_column('courses', sa.Column('price', sa.Integer(), nullable=False,
                                          server_default='0'))
    
    # Add created_at if it doesn't exist
    if 'created_at' not in columns:
        op.add_column('courses', sa.Column('created_at', sa.Date(), nullable=False, 
                                          server_default=sa.func.current_date()))
    
    # Add updated_at if it doesn't exist
    if 'updated_at' not in columns:
        op.add_column('courses', sa.Column('updated_at', sa.Date(), nullable=False, 
                                          server_default=sa.func.current_date()))
    
    # Add status if it doesn't exist
    if 'status' not in columns:
        op.add_column('courses', sa.Column('status', sa.String(), nullable=False, 
    ...
    constraints = []
    try:
        constraints = [con['name'] for con in inspector.get_check_constraints('courses')]
    except:
        pass
    ...
    try:
        if 'check_max_students_positive' not in constraints:
            op.execute('ALTER TABLE courses ADD CONSTRAINT check_max_students_positive CHECK (max_students >= 0)')
    except Exception as e:
        print(f"Error adding max_students constraint: {e}")
    
    try:
        if 'check_current_students_positive' not in constraints:
            op.execute('ALTER TABLE courses ADD CONSTRAINT check_current_students_positive CHECK (current_students >= 0)')
    except Exception as e:
        print(f"Error adding current_students constraint: {e}")
    
    try:
        if 'check_price_positive' not in constraints:
            op.execute('ALTER TABLE courses ADD CONSTRAINT check_price_positive CHECK (price >= 0)')
    except Exception as e:
        print(f"Error adding price constraint: {e}")
    
    try:
        if 'check_course_status_valid' not in constraints:
            op.execute("ALTER TABLE courses ADD CONSTRAINT check_course_status_valid CHECK (status IN ('active', 'inactive'))")
    except Exception as e:
...
def downgrade() -> None:
    ...
    for constraint_name in ['check_max_students_positive', 'check_current_students_positive', 
                           'check_price_positive', 'check_course_status_valid']:
