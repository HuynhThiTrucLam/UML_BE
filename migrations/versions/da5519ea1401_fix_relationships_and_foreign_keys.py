...
def upgrade() -> None:
    ...
    def fix_payment_method():
        if "payments" in tables:
            ...
            payment_method_col = next(
                (c for c in columns if c["name"] == "payment_method_id"), None
            )

            if payment_method_col and isinstance(
                payment_method_col["type"], postgresql.UUID
            ):
                op.alter_column(
                    "payments",
                    "payment_method_id",
    ...
    def add_vehicle_relationship():
        ...
        vehicle_fk = next(
            (c for c in constraints if c.get("referred_table") == "vehicles"), None
        )

        if not vehicle_fk:
            op.create_foreign_key(
                "schedules_vehicle_id_fkey",
                "schedules",
                "vehicles",
                ["vehicle_id"],
                ["id"],
    ...
    def fix_payment_course_registration_fk():
        ...
        course_reg_fk = next(
            (
                ...
                if c.get("referred_table") == "course_registrations"
                and "course_registration_id" in c.get("constrained_columns", [])
            ),
            None,
        )
