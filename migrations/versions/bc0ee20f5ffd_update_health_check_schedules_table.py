...
def upgrade() -> None:
    ...
    if "license_types" in inspector.get_table_names():
        ...
        id_column = next(
            (col for col in license_types_columns if col["name"] == "id"), None
        )
            ...
            try:
                ...
                if "licenses" in inspector.get_table_names():
                    for fk in inspector.get_foreign_keys("licenses"):
                        if fk.get(
                            "referred_table"
                        ) == "license_types" and "id" in fk.get("referred_columns", []):
                            op.drop_constraint(
                                fk.get("name"), "licenses", type_="foreignkey"
                            )
                    ...
                    if any(col["name"] == "license_type_id" for col in license_columns):
                        ...
                        op.add_column(
                            "licenses",
                            sa.Column(
                                "license_type_id", postgresql.UUID(), nullable=True
                            ),
                        )
                ...
                op.add_column(
                    "license_types",
                    sa.Column("id_new", postgresql.UUID(), nullable=True),
                )
                ...
                op.drop_constraint(
                    "license_types_pkey", "license_types", type_="primary"
                )
                ...
                op.alter_column(
                    "license_types", "id_new", new_column_name="id", nullable=False
                )
                op.create_primary_key("license_types_pkey", "license_types", ["id"])

                # Create the index
                op.create_index(
                    op.f("ix_license_types_id"), "license_types", ["id"], unique=False
                )
                ...
                if "licenses" in inspector.get_table_names():
                    try:
                        ...
                        print(
                            f"Warning: Could not create foreign key for licenses: {e}"
                        )
            except Exception as e:
        ...
        if "health_check_schedules" not in inspector.get_table_names():
            ...
            op.create_table(
                ...
                sa.CheckConstraint(
                    ...
                    name="check_schedule_in_future",
                ),
            )
            op.create_index(
                op.f("ix_health_check_schedules_id"),
                "health_check_schedules",
                ["id"],
                unique=False,
            )
            op.create_index(
                op.f("ix_health_check_schedules_license_type_id"),
                "health_check_schedules",
                ["license_type_id"],
                unique=False,
            )
            op.create_index(
                op.f("ix_health_check_schedules_address"),
                "health_check_schedules",
                ["address"],
                unique=False,
            )
            ...
            if "license_type_id" in column_names and not isinstance(
                column_types["license_type_id"], postgresql.UUID
            ):
                # Drop any foreign key constraints first
                for fk in inspector.get_foreign_keys("health_check_schedules"):
                    if "license_type_id" in fk.get("constrained_columns", []):
                        op.drop_constraint(
                            fk.get("name"), "health_check_schedules", type_="foreignkey"
                        )
                ...
                if col_name not in column_names:
                    if col_name == "id":
                        ...
                        op.add_column(
                            "health_check_schedules",
                            sa.Column(
                                "license_type_id", postgresql.UUID(), nullable=False
                            ),
                        )
                        ...
                        op.add_column(
                            "health_check_schedules",
                            sa.Column(
                                "scheduled_datetime", sa.DateTime(), nullable=False
                            ),
                        )
            ...
            try:
                ...
                print(
                    f"Info: check_schedule_in_future constraint might already exist: {e}"
                )
            ...
            if fk_name not in fk_names and not any(
                fk["referred_table"] == "license_types" for fk in foreign_keys
            ):
                try:
                    op.create_foreign_key(
                        fk_name,
...
def downgrade() -> None:
    ...
    try:
        op.drop_constraint(
            "check_schedule_in_future", "health_check_schedules", type_="check"
        )
    except:
        pass
    ...
    try:
        op.drop_constraint(
            "fk_health_check_schedules_license_type_id_license_types",
            "health_check_schedules",
            type_="foreignkey",
        )
    except:
    ...
    try:
        op.drop_index(
            op.f("ix_health_check_schedules_address"),
            table_name="health_check_schedules",
        )
    except:
        pass
    ...
    try:
        op.drop_index(
            op.f("ix_health_check_schedules_id"), table_name="health_check_schedules"
        )
    except:
        pass
