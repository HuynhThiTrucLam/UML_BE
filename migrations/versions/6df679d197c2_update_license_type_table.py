...
def upgrade() -> None:
    ...
    if "license_types" not in inspector.get_table_names():
        ...
        op.create_index(
            op.f("ix_license_types_type_name"),
            "license_types",
            ["type_name"],
            unique=True,
        )
        ...
        for col_name in required_columns:
            if col_name not in column_names:
                if col_name == "id":
                    ...
                    op.add_column(
                        "license_types", sa.Column("fee", sa.Integer(), nullable=False)
                    )
        ...
        try:
            op.create_check_constraint(
                "check_training_duration_positive",
                "license_types",
                "training_duration > 0",
            )
        except:
        ...
        try:
            op.create_check_constraint(
                "check_fee_non_negative", "license_types", "fee >= 0"
            )
        except:
            # Constraint may already exist
            pass
...
def downgrade() -> None:
    ...
    try:
        op.drop_constraint(
            "check_training_duration_positive", "license_types", type_="check"
        )
    except:
        pass
