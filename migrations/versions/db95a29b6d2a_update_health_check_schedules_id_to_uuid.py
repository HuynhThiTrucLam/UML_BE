        ...
        if id_column and not isinstance(id_column["type"], sa.dialects.postgresql.UUID):
            ...
            for table_name in inspector.get_table_names():
                for fk in inspector.get_foreign_keys(table_name):
                    if fk.get(
                        "referred_table"
                    ) == "health_check_schedules" and "id" in fk.get(
                        "referred_columns", []
                    ):
                        op.drop_constraint(
                            fk.get("name"), table_name, type_="foreignkey"
                        )

            try:
                # Drop indexes on id column first
                op.drop_index(
                    "ix_health_check_schedules_id", table_name="health_check_schedules"
                )
            ...
            op.add_column(
                ...
                sa.Column("id_new", sa.dialects.postgresql.UUID(), nullable=True),
            )
            ...
            op.drop_constraint(
                "health_check_schedules_pkey", "health_check_schedules", type_="primary"
            )
            ...
            op.alter_column(
                "health_check_schedules", "id_new", new_column_name="id", nullable=False
            )
            op.create_primary_key(
                "health_check_schedules_pkey", "health_check_schedules", ["id"]
            )
...
def downgrade() -> None:
    ...
    print(
        "WARNING: Downgrade from UUID to integer is not supported for health_check_schedules.id"
    )
    pass
