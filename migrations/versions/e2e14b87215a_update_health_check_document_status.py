...
def upgrade() -> None:
    ...
    if 'health_check_documents' not in inspector.get_table_names():
        ...
        op.create_table(
            ...
            sa.Column('health_check_id', sa.Integer(), nullable=False),  # Changed to Integer to match db schema
            sa.Column('document', sa.String(), nullable=False),
            sa.Column('status', sa.String(), nullable=False, server_default='pending'),
            sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.text('CURRENT_TIMESTAMP')),
            sa.ForeignKeyConstraint(['student_id'], ['students.id'], ondelete='CASCADE'),
            sa.ForeignKeyConstraint(['health_check_id'], ['health_check_schedules.id'], ondelete='CASCADE'),
            sa.PrimaryKeyConstraint('id'),
            sa.CheckConstraint("status IN ('registered', 'checked', 'pending')", name='check_valid_document_status')
        )
        op.create_index(op.f('ix_health_check_documents_id'), 'health_check_documents', ['id'], unique=False)
        op.create_index(op.f('ix_health_check_documents_student_id'), 'health_check_documents', ['student_id'], unique=False)
        op.create_index(op.f('ix_health_check_documents_health_check_id'), 'health_check_documents', ['health_check_id'], unique=False)
    else:
        # If the table exists, update the constraint
        try:
            op.drop_constraint('check_valid_document_status', 'health_check_documents', type_='check')
        except:
