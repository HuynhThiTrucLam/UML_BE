...
def upgrade():
    ...
    op.execute(
        "UPDATE courses SET uuid_id = gen_random_uuid()"
    )  # hoặc uuid_generate_v4()
    ...
    op.add_column(
        "course_registrations", sa.Column("uuid_course_id", sa.UUID(), nullable=True)
    )
    ...
    op.alter_column(
        "course_registrations", "uuid_course_id", new_column_name="course_id"
    )
