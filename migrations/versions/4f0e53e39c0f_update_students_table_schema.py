...
def upgrade() -> None:
    ...
    if 'students' in inspector.get_table_names():
        ...
        if not isinstance(columns.get('id', {}).get('type'), postgresql.UUID):
            ...
            dependent_fks = []
            dependent_tables = [
                'absent_forms', 'course_registrations', 'exam_results', 
                'licenses', 'payments', 'student_health_checks', 
            ...
            for table_name in dependent_tables:
                if table_name in inspector.get_table_names():
                    for fk in inspector.get_foreign_keys(table_name):
                        if fk.get('referred_table') == 'students' and 'id' in fk.get('referred_columns', []):
            ...
            op.add_column('students', sa.Column('user_id_new', postgresql.UUID(), nullable=True))
            
            # Step 2: Generate UUIDs for all existing records and create a mapping table
            op.execute("CREATE TEMP TABLE student_id_mapping AS SELECT id, uuid_generate_v4() as new_id FROM students")
            op.execute("CREATE INDEX ON student_id_mapping (id)")
            
            # Update the new ID columns using the mapping
            op.execute("UPDATE students SET id_new = student_id_mapping.new_id FROM student_id_mapping WHERE students.id = student_id_mapping.id")
            ...
            if 'users' in inspector.get_table_names() and isinstance(users_columns.get('id', {}).get('type'), postgresql.UUID):
            ...
            op.alter_column('students', 'user_id_new', new_column_name='user_id', nullable=False)
            ...
            op.add_column('students', sa.Column('course_of_study', sa.String(length=100), nullable=True))
            ...
            for table_name in dependent_tables:
                if table_name in inspector.get_table_names():
                    ...
                    op.add_column(table_name, sa.Column('student_id_new', postgresql.UUID(), nullable=True))
                    ...
                    op.alter_column(table_name, 'student_id_new', new_column_name='student_id', nullable=False)
