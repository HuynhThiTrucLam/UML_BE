...
from app.schemas.schedule import ScheduleCreate, ScheduleUpdate, ScheduleList, Schedule as ScheduleSchema
import uuid
...
def get_schedule(db: Session, start_time: str, end_time: str):
    ...
    schedules = db.query(Schedule).options(
        joinedload(Schedule.course).joinedload(Course.license_type)
    ).filter(
