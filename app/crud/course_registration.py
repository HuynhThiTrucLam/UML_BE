...
def create_course_registration(db: Session, course_registration: CourseRegistrationCreate):
    """
    ...
    try:
        ...
        health_check_doc = _create_health_check_document(db, student.id, course_registration)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create course registration: {str(e)}")


def _create_user_for_registration(db: Session, course_registration: CourseRegistrationCreate):
    """Create a new user for the registration"""
    # Use a default email if none provided
    email = course_registration.email if course_registration.email else f"{course_registration.identity_number}@example.com"
...
def _create_personal_information(db: Session, user_id: str, course_registration: CourseRegistrationCreate):
...
def _create_health_check_document(db: Session, student_id: str, course_registration: CourseRegistrationCreate):
...
def get_course_registration_by_id(db: Session, course_registration_id: uuid.UUID) -> CourseRegistrationSchema:
    """
    ...
    """
    db_course_registration = db.query(CourseRegistration).filter(CourseRegistration.id == course_registration_id).first()
    ...
    health_check_doc = db.query(HealthCheckDocumentModel).filter(
        HealthCheckDocumentModel.student_id == db_course_registration.student_id,
        HealthCheckDocumentModel.health_check_id == db_course_registration.student.health_check_documents[0].health_check_id if 
            db_course_registration.student and db_course_registration.student.health_check_documents else None
    ).first()
    ...
    if student and student.user:
        personal_doc = db.query(PersonalInforDocument).filter(
            PersonalInforDocument.user_id == student.user_id
        ).first()
