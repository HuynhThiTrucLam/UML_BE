...
def get_health_check_schedule(db: Session, health_check_schedule_id: uuid.UUID):
    ...
    """
    return (
        db.query(HealthCheckSchedule)
        .filter(HealthCheckSchedule.id == health_check_schedule_id)
        .first()
    )
...
def create_health_check_schedule(
    db: Session, health_check_schedule_in: HealthCheckScheduleCreate
):
    """
    ...
    health_check_schedule = HealthCheckSchedule(
        ...
        description=(
            health_check_schedule_in.description
            if health_check_schedule_in.description
            else None
        ),
        status=(
            health_check_schedule_in.status
            if hasattr(health_check_schedule_in, "status")
            else "scheduled"
        ),
...
def update_health_check_schedule(
    db: Session, schedule_id: uuid.UUID, schedule_in: HealthCheckScheduleUpdate
):
    """
...
def get_health_check_schedules(db: Session, skip: int = 0, limit: int = 100):
    ...
    health_check_schedules = (
        db.query(HealthCheckSchedule).offset(skip).limit(limit).all()
    )
