...
class HealthCheckDocument(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('registered', 'checked')", name="check_valid_document_status"
        ),
    )
    ...
    health_check_id = Column(
        PostgresUUID(as_uuid=True),
        ForeignKey("health_check_schedules.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    health_check = relationship(
        "HealthCheckSchedule",
        back_populates="health_check_documents",
        foreign_keys=[health_check_id],
    )
