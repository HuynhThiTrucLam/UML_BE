...
class Course(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "current_students >= 0", name="check_current_students_positive"
        ),
        CheckConstraint("price >= 0", name="check_price_positive"),
        CheckConstraint(
            "status IN ('active', 'inactive')", name="check_course_status_valid"
        ),
    )
    ...
    license_type_id = Column(
        UUID,
        ForeignKey("license_types.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    health_check_schedules = relationship(
