...
from app.schemas.health_check_schedule import (
    HealthCheckSchedule,
    HealthCheckScheduleCreate,
    HealthCheckScheduleUpdate,
    HealthCheckScheduleList,
)
...
@router.get(
    "/{schedule_id}",
    response_model=HealthCheckSchedule,
    summary="Get Health Check Schedule by ID",
)
def get_health_check_schedule(
    ...
    """
    health_check_schedule = crud_health_check_schedule.get_health_check_schedule(
        db=db, health_check_schedule_id=schedule_id
    )
...
@router.post(
    "/", response_model=HealthCheckSchedule, status_code=status.HTTP_201_CREATED
)
def create_health_check_schedule(
    ...
    health_check_schedule = crud_health_check_schedule.create_health_check_schedule(
        db=db, health_check_schedule_in=schedule_in
    )
    return health_check_schedule
...
@router.get(
    "/", response_model=HealthCheckScheduleList, summary="List Health Check Schedules"
)
def list_health_check_schedules(
    ...
    """
    schedules = crud_health_check_schedule.get_health_check_schedules(
...
@router.put(
    "/{schedule_id}",
    response_model=HealthCheckSchedule,
    summary="Update Health Check Schedule",
)
def update_health_check_schedule(
    ...
    print(
        f"Update request for schedule {schedule_id}: {schedule_in.model_dump(exclude_unset=True)}"
    )

    try:
        updated_schedule = crud_health_check_schedule.update_health_check_schedule(
            db=db, schedule_id=schedule_id, schedule_in=schedule_in
        )
        if updated_schedule is None:
            raise HTTPException(
                status_code=404, detail="Health check schedule not found"
            )
...
@router.delete(
    "/{schedule_id}",
    response_model=HealthCheckSchedule,
    summary="Delete Health Check Schedule",
)
def delete_health_check_schedule(
    ...
    """
    result = crud_health_check_schedule.delete_health_check_schedule(
        db=db, health_check_schedule_id=schedule_id
    )
    if result is None:
        raise HTTPException(status_code=404, detail="Health check schedule not found")
